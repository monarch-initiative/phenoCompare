...
Define a boolean function that takes two sets as its arguments and returns TRUE if the two sets overlap, FALSE otherwise.
```{r}
overlap <- function(set1, set2) {
  length(set_intersection(set1, set2)) > 0
}
```
Repeat __params\$num.iter__ times: choose at random a set of genes from __params\$gene.info__ equal in size to $G_{2}$ and disjoint from $G_{1}$. Find the set of associated phenotypes $P_{3}$ and calculate the Jaccard similarity with $P_{1}$. Count how many times $P_{1}$'s similarity to $P_{3}$ is $\geq$ its similarity to $P_{2}$.
