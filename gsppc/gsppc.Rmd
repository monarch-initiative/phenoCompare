...
3. Iterate $n$ times:
+ Select from all human protein-coding genes a set $G_{3}$ that is disjoint from $G_{1}$ and equal in size to $G_{2}$.
...
### Load libraries
Meyer D and Hornik K (2009). “Generalized and Customizable Sets in R.” _Journal of Statistical Software_, 31(2), pp. 1–27. doi: 10.18637/jss.v031.i02.
```{r}
...
pheno.2.gene <- read.table(params$pheno.to.gene, sep = "\t", skip = 1,
...
lines.read
```
The blank line indicates the end of the first set and the beginning of the second set.

### Create gene sets
Create two gene sets, one for GPI pathway genes and the other for genes that code the GPI achored proteins. Each is a set of ENTREZ gene ids, treated as integers. Parse the strings read from __params$to.compare__ to create sets of integers.
```{r}
collector <- c()
gene.sets <- list()
...
```
### Create phenotype sets
For each set of genes, create the corresponding set of phenotypes that includes every phenotype reported for each gene.

First define function that takes gene id as parameter and returns set of phenotypes associated with that gene.
