...
params:
  ...
  gene.to.pheno: "ALL_SOURCES_ALL_FREQUENCIES_genes_to_phenotype.txt"
  num.iter: 10000
---
...
```
* file mapping gene id/name to HPO phenotype id/name: [ALL_SOURCES_ALL_FREQUENCIES_genes_to_phenotype.txt](http://compbio.charite.de/jenkins/job/hpo.annotations.monthly/lastSuccessfulBuild/artifact/annotation/ALL_SOURCES_ALL_FREQUENCIES_genes_to_phenotype.txt)
...
gene.2.pheno <- read.delim(params$gene.to.pheno, header = FALSE, sep = "\t", skip = 1,
                           col.names = c("Gene.ID", "Gene.Name", "HPO.Name", "HPO.ID"),
...
find.phenotypes <- function(gene.id) {
  phenotypes <- subset(gene.2.pheno, select=HPO.ID, subset=(Gene.ID==gene.id))
